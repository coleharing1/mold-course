...
const getCategoryImage = (category: string): string => {
  ...
  const fallback: string = categoryImages['supplements'] ?? 'https://images.unsplash.com/photo-1596116762424-01639b22c9f0?w=400&h=400&fit=crop'
  const value = categoryImages[category as keyof typeof categoryImages]
  return typeof value === 'string' && value.length > 0 ? (value as string) : fallback
}
...
export default function ProductsPage() {
  ...
  // Lazy-load the optional name→image mapping generated by the scraper
  if (typeof window !== 'undefined' && !(window as any).__PRODUCT_NAME_IMG_MAP__) {
    fetch('/product-images-by-name.json')
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (data && typeof data === 'object') {
          ;(window as any).__PRODUCT_NAME_IMG_MAP__ = data
        }
      })
      .catch(() => {})
  }
  ...
  const resolveImage = (product: Product) => {
    // 1) Try name-based mapping from public/product-images-by-name.json if present
    // 2) Fallback to curated enhanced map
    // 3) Fallback to category placeholder

    // Inline cache to avoid repeated fetches
    const w = typeof window !== 'undefined' ? (window as any) : undefined
    const byName: Record<string, string> | undefined = w?.__PRODUCT_NAME_IMG_MAP__
    if (byName) {
      const slug = product.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
      if (byName[slug]) return byName[slug]
    }

    // Curated mapping (guaranteed local enhanced 400x400)
                      ...
                      <div className="mb-3 flex items-center justify-between">
                        {product.price && (
                          <div className="flex items-center gap-2">
                            <span className="text-xl font-bold text-gray-900">{product.price}</span>
                            {product.discount && (
                              <Badge className="bg-red-500 text-xs text-white">-{product.discount}%</Badge>
                            )}
                          </div>
                        )}
                        <Badge className={cn('text-xs', getPriorityColor(product.priority))}>
